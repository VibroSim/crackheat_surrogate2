    ...
    def eval_formula_values(self,new_positions):
        ...
        if self.trend_formula==[u'~x + I(x^2)']: # Simple linear + quadratic
            ...
            formula_values=np.array((np.ones(new_positions.shape[0],dtype='d'),
                                     ...
                                     new_positions["log_crack_model_shear_factor"],
                                     new_positions["mu_norm"]**2.0,
                                     new_positions["log_msqrtR_norm"]**2.0,
                                     new_positions["log_crack_model_shear_factor"]**2.0,
                                     new_positions["mu_norm"]*new_positions["log_msqrtR_norm"],
                                     new_positions["mu_norm"]*new_positions["log_crack_model_shear_factor"],
                                     new_positions["log_msqrtR_norm"]*new_positions["log_crack_model_shear_factor"]),dtype='d').T
            pass
        else:
    ...
    def eval_formula_values_deriv_k(self,new_positions,k):
        ...
        if self.trend_formula==['~mu_norm + log_msqrtR_norm + log_crack_model_shear_factor + I(mu_norm^2) + ',
            ...
            if k=="mu_norm":
                ...
                pass
            elif k=="log_crack_model_shear_factor":
                formula_values_deriv_k=np.array((np.zeros(new_positions.shape[0],dtype='d'),
                                                 np.zeros(new_positions.shape[0],dtype='d'),
                                                 np.zeros(new_positions.shape[0],dtype='d'),
                                                 np.ones(new_positions.shape[0],dtype='d'),
                                                 np.zeros(new_positions.shape[0],dtype='d'),
                                                 np.zeros(new_positions.shape[0],dtype='d'),
                                                 2.0*new_positions["log_crack_model_shear_factor"],
                                                 np.zeros(new_positions.shape[0],dtype='d'),
                                                 new_positions["mu_norm"],
                                                 new_positions["log_msqrtR_norm"],
                ),dtype='d').T
                ...
                raise ValueError("k can only be (normalized) mu, log_msqrtR or log_crack_model_shear_factor. ")
            pass
        else:
            raise ValueError("Unknown formula: %s" % ("".join(self.trend_formula)))
...
"""
        ...
        new_positions_covariance = covMatern5_2(self.X,new_positions[["mu_norm","log_msqrtR_norm","log_crack_model_shear_factor"]].values,self.covariance_param,np.sqrt(5.0),self.covariance_sd2)  # The sqrt(5.0) is the scaling factor for Matern5_2
    ...
    def evaluate_derivative(self,new_positions,k,accel_trisolve_devs=None):
        ...
        new_positions_covariance_deriv_k = covMatern5_2_deriv_x2_k(self.X,new_positions[["mu_norm","log_msqrtR_norm","log_crack_model_shear_factor"]].values,self.covariance_param,np.sqrt(5.0),self.covariance_sd2,k)  # The sqrt(5.0) is the scaling factor for Matern5_2
...
class denormalized_surrogate_shear(surrogate_model_shear):
    ...
    def evaluate_derivative(self,new_positions,k,accel_trisolve_devs=None):
        ...
        k_norm = {"mu": "mu_norm", "log_msqrtR": "log_msqrtR_norm", "log_crack_model_shear_factor": "log_crack_model_shear_factor" }[k]
...
class nonnegative_denormalized_surrogate_shear(denormalized_surrogate_shear):
    # Class representing denormalized surrogate model
...
def load_denorm_surrogates_shear_from_jsonfile(surrogates_jsonfile,nonneg=True,**kwargs):
    ...
    if nonneg:
        denormalized_surrogate_class = nonnegative_denormalized_surrogate_shear
        pass
    else:
        denormalized_surrogate_class = denormalized_surrogate_shear
        pass
